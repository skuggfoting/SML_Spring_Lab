...
public final class Main {
	...
	public static void main(String[] args) {
		...
		try (AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext()) {
			...
			workItemService.addIssue("2001", issue1);
			System.out.println("\nAdd Issue to WorkItem:\n" + workItem1);
...
//			teamService.addUser("PR", user1);
			...
			User userUpdate = userService.getByUserNumber("1003");
			userUpdate.setStatus("Inactive");
			userService.save(userUpdate);
			System.out.println("\nUpdated status and inactivation for User:\n" + userService.getByUserNumber("1003").getStatus());
			...
			WorkItem workItemUpdate = workItemService.getByWorkItemNumber("2003");
			workItemUpdate.setStatus("Inactive");
			workItemService.save(workItemUpdate);
			System.out.println("\nUpdated status and inactivation for WorkItem:\n" + workItemService.getByWorkItemNumber("2003").getStatus());
			
			
			// User queries: /////////////////////////////////////////////////////////////////////////////////////////////////

			// Find User by userNumber
			System.out.println("\nUserNumber:\n" + userService.getByUserNumber("1001"));

			// Find User by firstName and lastName
			System.out.println("\nUser:");
			userService.getByFirstNameAndLastName("Silvia", "Bernadotte").forEach(System.out::println);
			// Find USer by firstName and lastName and username
			System.out.println("\nShit loads of different names:");
			userService.getByFirstNameAndLastNameAndUsername("Carl Gustav", "Bernadotte", "CG16-12345").forEach(System.out::println);

			// Get all Users from a Team
			System.out.println("\nAll Users from a Team:");
			userService.getUsersByTeam("Hovet").forEach(System.out::println);
			...
			workItemService.getByStatus("Started").forEach(System.out::println);

			// Get all workItems for a Team
			System.out.println("\nAll WorkItems for a specified Team:");
			workItemService.getByTeam("Hovet").forEach(System.out::println);

			// Get all workItem for a User
			System.out.println("\nAll WorkItems from a User:");
			userService.getWorkItemsByUser("CG16-12345").forEach(System.out::println);

			// Get all workItems containing a specified text-string
			System.out.println("\nAll WorkItem containing specified text-string:");
			workItemService.getByDescriptionContaining("ugly").forEach(System.out::println);
